...
class TestCommandReference:
    """docs/commands.md is hand-written; this stops it drifting from the real CLI."""
    ...
    def test_every_command_is_documented(self, runner) -> None:
        import click

        text = self._text()
        with click.Context(cli.main) as context:
            commands = cli.main.list_commands(context)
        undocumented = [c for c in commands if f"`{c}`" not in text]
        assert not undocumented, f"missing from docs/commands.md: {undocumented}"

    def test_every_option_is_documented(self, runner) -> None:
        import click

        text = self._text()
        missing: list[str] = []
        with click.Context(cli.main) as context:
            for name in cli.main.list_commands(context):
                command = cli.main.get_command(context, name)
                for param in command.params:  # type: ignore[union-attr]
                    flags = [o for o in getattr(param, "opts", []) if o.startswith("--")]
                    # Help is generated by click and needs no entry of its own.
                    if not flags or flags[0] in ("--help",):
                        continue
                    if not any(flag in text for flag in flags):
                        missing.append(f"{name} {flags[0]}")
        assert not missing, f"missing from docs/commands.md: {missing}"
